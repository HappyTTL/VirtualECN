...
if {$enableNam != 0} {
	...
	set namfile [open outNewRenoVSUDPExpS1.nam w]
	$ns namtrace-all $namfile
}

set tf [open outNewRenoVSUDPExpS1.tr w]
$ns trace-all $tf

set mytracefile [open mytracefileNewRenoVSUDPExpS1.tr w]
set throughputfile [open thrfileNewRenoVSUDPExpS1.tr w]
...
proc finish {} {
	...
	if {$enableNam != 0} {
		close $namfile
		exec nam outNewRenoVSUDPExpS1.nam &
	}
	exit 0
}
...
# $pareto1 attach-agent $udp(3)
# $pareto2 attach-agent $udp(4)

# $pareto1 set paketSize_ 500
# $pareto1 set burst_time_ 200ms
# $pareto1 set idle_time_ 800ms
...
# $pareto2 set idle_time_ 100ms
# $pareto2 set rate_ 100Mb
...
$ns at $UDPstartT(0) "$exp1 start"
$ns at [expr $simulationTime] "$exp1 stop"

$ns at $UDPstartT(1) "$exp2 start"
$ns at [expr $simulationTime] "$exp2 stop"

$ns at $UDPstartT(2) "$exp3 start"
...
puts "Throughput = [expr ($stopPacketCount-$startPacketCount)/(1024000*($stopMeasurementTime-$startMeasurementTime))*$packetSize*8] Mbps"
}
