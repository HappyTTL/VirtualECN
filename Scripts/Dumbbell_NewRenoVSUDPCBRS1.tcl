...
if {$enableNam != 0} {
	...
	set namfile [open outNewRenoVSUDPCBRS1.nam w]
	$ns namtrace-all $namfile
}

set tf [open outNewRenoVSUDPCBRS1.tr w]
$ns trace-all $tf

set mytracefile [open mytracefileNewRenoVSUDPCBRS1.tr w]
set throughputfile [open thrfileNewRenoVSUDPCBRS1.tr w]
...
proc finish {} {
	...
	if {$enableNam != 0} {
		close $namfile
		exec nam outNewRenoVSUDPCBRS1.nam &
	}
	exit 0
}
...
$cbr1 attach-agent $udp(0)
$cbr2 attach-agent $udp(1)
$cbr3 attach-agent $udp(2)
...
$cbr3 set type_ CBR
$cbr3 set packet_size_ 10
$cbr3 set rate_ 100mb
$cbr3 set random_ false

$ns at $UDPstartT(0) "$cbr1 start"
$ns at [expr $simulationTime] "$cbr1 stop"

$ns at $UDPstartT(1) "$cbr2 start"
$ns at [expr $simulationTime] "$cbr2 stop"

$ns at $UDPstartT(2) "$cbr3 start"
$ns at [expr $simulationTime] "$cbr3 stop"
